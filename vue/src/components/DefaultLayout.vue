...
<template>
    ...
    <Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
        ...
        <div class="flex h-16 items-center justify-between">
            ...
            <div class="hidden md:block">
                <div class="ml-10 flex items-baseline space-x-4">
                    <router-link v-for="item in navigation" :key="item.name" :to="item.to" :class=" [this.$route.name === item.to.name ? '' :'text-gray-300 hover:bg-gray-700 hover:text-white', 'px-3 py-2 rounded-md text-sm font-medium']" active-class="bg-gray-900 text-white">{{ item.name }}</router-link>
                </div>
            </div>
            </div>
            <div class="hidden md:block">
            <div class="ml-4 flex items-center md:ml-6">
                ...
                <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                    ...
                    <MenuItem v-slot="{ active }">
                        <a @click="logout" :class="['block px-4 py-2 cursor-pointer text-sm text-gray-700']">Log out</a>
                    </MenuItem>
                    </MenuItems>
                </transition>
                </Menu>
        ...
        <div class="border-t border-gray-700 pt-4 pb-3">
            ...
            <DisclosureButton as="a" @click="logout" class="block rounded-md cursor-pointer first-letter: px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white">Log out</DisclosureButton>
            </div>
        </div>
        </DisclosurePanel>
    </Disclosure>
